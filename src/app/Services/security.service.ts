...
export class SecurityService {
  ...
  ifStillValid(): Observable<ProjectDetailsInterface> {
    // checks if the token from localstorage is valid or not
  ...
  syncAll(username: string, pname: string) {
    if (localStorage.getItem("auditToken") == null && this.token.Jwt != "") {
      console.log("Not excutiable  ");
      localStorage.setItem("auditToken", this.token.Jwt);
    } else if (
      this.token.Jwt == "" &&
      localStorage.getItem("auditToken") != null
    ) {
  ...
  checkAuthFromLocal(spath: string, epath: string) {
    if (localStorage.getItem("auditToken") == null) {
      if (this.getLoginStatus()) {
        // update localstorage
        console.log("check auth if condn ....");

        localStorage.setItem("auditToken", this.token.Jwt);
        // do nothing
      } else {
        console.log("check auth else condn ....");
      ...
      this.ifStillValid().subscribe(
        //validate once if it works or not
        (data) => {
          fetch = data;
          console.log("data ****");
