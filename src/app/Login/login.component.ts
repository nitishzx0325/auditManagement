...
export class LoginComponent implements OnInit {
    ...
    this.securityService.healthCheck().subscribe(
      ...
      () => {
        this.securityService
          .createSecuritytokenObservable(this.username, this.password)
          .subscribe(
            (data) => {
              //checks
              if (data.includes(".")) {
            ...
            () => {
              ...
              if (this.securityService.getLoginStatus()) {
                this.securityService
                  .validateToken(this.securityService.getSecurityToken())
                  .subscribe(
                    (data) => {
                      //checks
  ...
  ngOnInit(): void {
    ...
    this.securityService.healthCheck().subscribe(
      (data) => {
        console.log("Helth check completed !!!!!");
      },
      (err) => {
        this.securityService.resetAll();
        console.log("Helth check errr   completed !!!!!");
        this.router.navigate(["error"]);
      },
      () => {
        //init jtwtoken in not localstorage
        console.log("Helth check Auth    completed !!!!!");
